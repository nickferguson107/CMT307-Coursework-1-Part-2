...
def create_feature_arrays(dataset, vocabulary):
    ...
    for review in dataset:
        ...
        X_raw.append(x)
        y_raw.append(review[1])

    X = np.asarray(X_raw, dtype = np.int8)
...
def reduce_dimensions(X, y, target_dimensions):
    ...
    new_X = np.asarray(reducer.transform(X), dtype = np.int8)
    
    return reducer, new_X
...
if __name__ == '__main__':
    ...
    train_pos = process_raw('datasets_coursework1\\IMDb\\train\\imdb_train_pos.txt')
    train_neg = process_raw('datasets_coursework1\\IMDb\\train\\imdb_train_neg.txt')
    test_pos = process_raw('datasets_coursework1\\IMDb\\test\\imdb_test_pos.txt')
    test_neg = process_raw('datasets_coursework1\\IMDb\\test\\imdb_test_neg.txt')
    dev_pos = process_raw('datasets_coursework1\\IMDb\\dev\\imdb_dev_pos.txt')
    dev_neg = process_raw('datasets_coursework1\\IMDb\\dev\\imdb_dev_neg.txt')
